...
const seed = async () => {
  try {
    ...
    for (const data of modulePermissionData) {
      ...
      if (moduleExits) {
        module = await Module.findByIdAndUpdate(moduleExits.id, {
          name: data.name,
      ...
      if (module) {
        if (permissionsData) {
          for (const data of permissionsData) {
            ...
            const permissionExits = await Permission.findOne({
              displayName: data.displayName,
            });
            if (permissionExits) {
              await Permission.findByIdAndUpdate(permissionExits.id, {
                name: data.name,
                displayName: data.displayName,
              });
